...
/*This function is called whenever a user clicks a color, uses the (color) system picker,
or direct enters a named color or hex value into the color-widget's input box. */
function colorElements(cObj) {
	...
	if(document.getElementById("pageBackground-cb").checked) {
        ...
        //Also color the background of that plug-in font-selector, to match new background 
        var fsDrop = document.getElementsByClassName("fs-drop");
        fsDrop[0].style.background = cObj.toHexString();

        //update the palette table information
	...
	if(document.getElementById("h4-cb").checked) {
        var headerArray = $("H1,H2,H3,H4,H5").not(".noColor");
...
//User has entered garbage into the color-widget input box!
function wrongInput() {
...
//clear the color-widget input box.
...
//user uses a keyboard not a mouse!
...
/*  Taken from internet -- 
This fun-ction automatically blends (interpolates) two different colors to a chosen degree.  
This app uses it whenever a user changes the background color, to mix the new BG color
with a fixed touch of very light grey for the panels.  We should do more tricks with this!  
*/
...
/*creates and downloads a file from any text and a desired title.  Creates a link 
object (anchor) whose target is the file into the DOM, 'clicks' itself, and removes 
itself!  Very Clever, taken from internet*/
...
//builds (concatenates) the CSS Stylesheet string, piece by piece, using data shown on screen.   
function buildStylesheet() {
    ...
    var spacelessCaptionFont = spaceToPlus(captionFont);
...
//wired to onClick Event of the Big Download Button.
function downloadStylesheetTest() {
	download("YourVisualCssTheme.css", buildStylesheet());
}

/*This function is wired to the change Event of the font selector (drop-down)
and -- like colorElements -- runs through the check-list of elements and changes
the fonts of any that are font-changeable and check-marked */
...
/* functions below this line were taken from w3schools original color-picker page, 
then modified as needed for use in visualcss.  See also w3color.js */
